...
test('execute returns collection with month keys and amounts', function () {
...
test('execute returns normal and credit_card amounts per month', function () {
    $user = User::factory()->create();
    $account = Account::factory()->create(['user_id' => $user->id]);
    $expenseCategory = Category::factory()->create(['type' => TransactionTypeEnum::EXPENSE]);
...
test('execute returns all months in range with zeros when no transactions', function () {
    $user = User::factory()->create();
    ...
    $result->each(function ($row) {
        ...
        expect($row['normal'])->toBe(0.0);
        expect($row['credit_card'])->toBe(0.0);
    });
});
...
test('execute excludes reconciliation transactions', function () {
    ...
    expect($result->first()['total'])->toBe(100.0);
    expect($result->first()['normal'])->toBe(100.0);
    expect($result->first()['credit_card'])->toBe(0.0);
});
...
test('execute includes only expense category transactions', function () {
    ...
    expect($result->first()['total'])->toBe(200.0);
    expect($result->first()['normal'])->toBe(200.0);
});
...
test('execute uses start of month and end of month from date strings', function () {
    ...
    expect($result->first()['total'])->toBe(30.0);
...
test('execute spans multiple months with correct totals per month', function () {
    ...
    expect($july)->toMatchArray(['total' => 100.0, 'normal' => 100.0, 'credit_card' => 0.0]);

    $august = $result->firstWhere('month', '2025-08');
    expect($august)->toMatchArray(['total' => 50.0, 'normal' => 0.0, 'credit_card' => 50.0]);

    $september = $result->firstWhere('month', '2025-09');
    expect($september)->toMatchArray(['total' => 25.0, 'normal' => 25.0, 'credit_card' => 0.0]);
});
